...
function ScheduleEditDialog({ open, onClose, barberId, barberName, selectedDate, initialAction }) {
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (appointmentToCancel) {
                await appointmentService.updateAppointment(appointmentToCancel, { nuevoEstado: 'cancelada' });
    ...
    return (
        <>
                                            ...
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, borderBottom: '1px solid #eee' }}>
                                                ...
                                                {slot.cita_id && (
                                                    <Button
                                                        ...
                                                        color="error"
                                                        size="small"
                                                        onClick={() => handleOpenCancelConfirm(slot.cita_id)}
                                        ...
                                        <Grid item xs={12} key={block.id || block.hora_inicio_24h}>
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, borderBottom: '1px solid #eee' }}>
                                                ...
                                                <Button
                                                    ...
                                                    color="primary"
                                                    size="small"
                                                    onClick={() => handleOpenUnblockConfirm(block.id)}
            ...
            <Dialog open={openConfirmModal} onClose={handleCloseConfirm} maxWidth="xs">
                <DialogTitle sx={{ backgroundColor: '#1a202c', color: '#D4AF37' }}>
                    Confirmación
                </DialogTitle>
                <DialogContent>
                    <Typography sx={{ mt: 2 }}>
                        {appointmentToCancel && '¿Estás seguro de que quieres cancelar esta cita y liberar el horario?'}
                        {slotToDelete && '¿Estás seguro de que quieres eliminar este horario disponible?'}
                        {blockToUnblock && '¿Estás seguro de que quieres liberar este bloqueo de horario?'}
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ p: 2 }}>
                    <Button onClick={handleCloseConfirm} sx={{ color: '#f44336', '&:hover': { backgroundColor: 'rgba(244, 67, 54, 0.1)' } }}>
                        Cancelar
                    </Button>
                    <Button
